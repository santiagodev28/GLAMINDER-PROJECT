...
class BusinessController {
    ...
    static async updateBusiness(req, res) {
        try {
            ...
            const updatedBusiness = await Business.updateBusiness(negocio_id, updateData);
    ...
    static async changeBusinessState(req, res) {
        try {
            ...
            const result = await Business.changeBusinessState(negocio_id, nuevo_estado);
            res.json(result);
        } catch (error) {
            if (error.message === "Negocio no encontrado") {
                return res.status(404).json({ error: error.message });
    ...
    static async deleteBusiness(req, res) {
        try {
            ...
            if (!canDelete) {
                return res.status(400).json({ error: "No se puede eliminar el negocio porque tiene tiendas activas." });
            }
    ...
    static async getBusinessStats(req, res) {
        try {
            ...
            console.error("Error al obtener estadísticas del negocio:", error.message);
            res.status(500).json({ error: error.message });
        }
    }
