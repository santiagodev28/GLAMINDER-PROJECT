...
class EmployeeController {
    ...
    static async updateEmployee(req, res) {
        try {
            ...
            const updatedEmployee = await Employee.updateEmployee(empleado_id, updateData);
    ...
    static async changeEmployeeState(req, res) {
        try {
            ...
            const result = await Employee.changeEmployeeState(empleado_id, nuevo_estado);
            res.json(result);
        } catch (error) {
            if (error.message === "Empleado no encontrado") {
                return res.status(404).json({ error: error.message });
    ...
    static async reactivateEmployee(req, res) {
        try {
            const { empleado_id } = req.params;
            const reactivatedEmployee = await Employee.reactivateEmployee(empleado_id);
    ...
    static async getEmployeesBySpecialty(req, res) {
        try {
            ...
            console.error("Error al obtener empleados por especialidad:", error.message);
            res.status(500).json({ error: error.message });
        }
    }
    ...
    static async getAvailableEmployees(req, res) {
        try {
            ...
            const employees = await Employee.getAvailableEmployees(tienda_id, fecha, horario_id);
            res.json(employees);
        } catch (error) {
    ...
    static async getEmployeeStats(req, res) {
        try {
            ...
            const stats = await Employee.getEmployeeStats(empleado_id, fecha_inicio, fecha_fin);
            res.json(stats);
        } catch (error) {
            console.error("Error al obtener estadísticas del empleado:", error.message);
            res.status(500).json({ error: error.message });
        }
    }
