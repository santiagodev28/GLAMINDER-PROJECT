...
class Business {
    ...
    static async getBusinessByEmail(negocio_correo) {
        ...
        if (!result.success) {
            throw new Error(
                `Error al buscar negocio por email: ${result.error}`
            );
        }
    ...
    `;
        ...
        if (!result.success) {
            throw new Error(
                `Error al obtener tiendas del negocio: ${result.error}`
            );
        }
    ...
    static async updateBusiness(negocio_id, updateData) {
        ...
        if (
            negocio_correo &&
            negocio_correo !== existingBusiness.negocio_correo
        ) {
            const duplicateEmail = await this.getBusinessByEmail(
                negocio_correo
            );
            if (duplicateEmail) {
                throw new Error(
    ...
    static async changeBusinessState(negocio_id, nuevo_estado) {
        ...
        const query =
            "UPDATE negocios SET negocio_estado = ? WHERE negocio_id = ?";
        const result = await executeQuery(query, [nuevo_estado, negocio_id]);

        if (!result.success) {
            throw new Error(
                `Error al cambiar estado del negocio: ${result.error}`
            );
        }
