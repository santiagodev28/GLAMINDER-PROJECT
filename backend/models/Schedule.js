...
class Schedule {
  ...
  static async getAllSchedules(filters = {}) {
    let query = `
      SELECT 
        h.*,
        e.empleado_id,
        ue.usuario_nombre AS empleado_nombre,
        ue.usuario_apellido AS empleado_apellido,
        t.tienda_nombre,
        n.negocio_nombre,
    ...
    if (filters.horario_estado !== undefined) {
      conditions.push('h.horario_estado = ?');
    ...
    query += `
      ...
      ORDER BY h.horario_dia ASC, h.horario_hora_inicio ASC
    `;
  ...
  static async createSchedule(scheduleData) {
    ...
    if (!horario_dia || !horario_hora_inicio || !horario_hora_fin) {
      throw new Error('Los campos horario_dia, horario_hora_inicio y horario_hora_fin son obligatorios');
    }

    // Validar día de la semana
    if (!Object.values(this.DAYS).includes(horario_dia.toLowerCase())) {
      throw new Error(`Día inválido: ${horario_dia}. Debe ser uno de: ${Object.values(this.DAYS).join(', ')}`);
    }

    // Validar horarios
    const validation = this.validateTimeRange(horario_hora_inicio, horario_hora_fin);
    ...
    try {
      ...
      if (empleado_id) {
        ...
        const hasConflict = await this.checkScheduleConflict(empleado_id, horario_dia, horario_hora_inicio, horario_hora_fin);
        if (hasConflict) {
          throw new Error('El empleado ya tiene un horario que se superpone en este día y horario');
        }
      }
  ...
  static async updateSchedule(horario_id, updateData) {
    ...
    if (horario_dia && !Object.values(this.DAYS).includes(horario_dia.toLowerCase())) {
    ...
    if (empleado_id && empleado_id !== existingSchedule.empleado_id) {
      ...
      const newStart = horario_hora_inicio || existingSchedule.horario_hora_inicio;
      const newEnd = horario_hora_fin || existingSchedule.horario_hora_fin;

      const hasConflict = await this.checkScheduleConflict(empleado_id, newDay, newStart, newEnd, horario_id);
      if (hasConflict) {
        throw new Error('El empleado ya tiene un horario que se superpone en este día y horario');
      }
    }
    ...
    if ((horario_hora_inicio && horario_hora_inicio !== existingSchedule.horario_hora_inicio) ||
        (horario_hora_fin && horario_hora_fin !== existingSchedule.horario_hora_fin)) {
      const hasFutureCitas = await this.hasFutureAppointments(horario_id);
      if (hasFutureCitas) {
        throw new Error('No se puede modificar el horario porque tiene citas programadas');
      }
    }
  ...
  static async changeScheduleState(horario_id, nuevo_estado) {
    ...
    if (nuevo_estado === this.STATES.INACTIVE) {
      const hasFutureCitas = await this.hasFutureAppointments(horario_id);
      if (hasFutureCitas) {
        throw new Error('No se puede desactivar un horario con citas programadas');
      }
    }

    const query = 'UPDATE horarios SET horario_estado = ?, fecha_modificacion = NOW() WHERE horario_id = ?';
  ...
  static async deleteSchedule(horario_id) {
    ...
    if (hasCitas) {
      throw new Error('No se puede eliminar un horario que tiene citas asociadas. Considera desactivarlo en su lugar.');
    }
  ...
  static async getSchedulesByDay(horario_dia, filters = {}) {
    return await this.getAllSchedules({ ...filters, horario_dia: horario_dia.toLowerCase() });
  }
  ...
  static async validateEmployee(empleado_id) {
    const query = 'SELECT empleado_id, empleado_estado FROM empleados WHERE empleado_id = ?';
  ...
  static async checkScheduleConflict(empleado_id, dia, hora_inicio, hora_fin, exclude_horario_id = null) {
    let query = `
      SELECT horario_id 
      FROM horarios 
      WHERE empleado_id = ? 
  ...
  static async getScheduleStats(filters = {}) {
    ...
    if (filters.empleado_id) {
      query += ' LEFT JOIN empleados e ON h.empleado_id = e.empleado_id LEFT JOIN tiendas t ON e.tienda_id = t.tienda_id';
    ...
    if (!result.success) {
      throw new Error(`Error al obtener estadísticas de horarios: ${result.error}`);
    }
  ...
  static async getMostUsedSchedules(limit = 10, filters = {}) {
    ...
    if (!result.success) {
      throw new Error(`Error al obtener horarios más utilizados: ${result.error}`);
    }
  ...
  static formatScheduleResponse(schedule) {
    ...
    return {
      ...
      slots_disponibles: Math.floor(durationMinutes / (schedule.horario_duracion_cita || 30)),
  ...
  static generateTimeSlots(horario_hora_inicio, horario_hora_fin, duracion_cita = 30) {
    const slots = [];
